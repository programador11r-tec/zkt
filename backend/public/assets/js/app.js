...
async function renderInvoices() {
    ...
    tbody.querySelectorAll('[data-action="invoice"]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          const pnMsg = (pnJson?.message || pnJson?.msg || pnJson?.detail || '').toString().trim();
          const pnCode = (pnJson?.code ?? pnJson?.ret ?? pnJson?.status ?? null);
          const pnRaw = (js.pay_notify_raw || '').toString().trim();
          const pnHttp = js.pay_notify_http_code ? `HTTP ${js.pay_notify_http_code}` : 'HTTP —';

          const pnDetail = (
            pnMsg
              ? `${pnMsg}${pnCode != null ? ` (code ${pnCode})` : ''}`
              : (pnRaw ? pnRaw : 'Sin detalle')
          ).slice(0, 900);

          if (payFail) {
            // amarillo: apertura manual requerida
            showToast({
              variant: 'warning',
              title: 'PayNotify falló — Apertura manual requerida',
              message: `${js.pay_notify_error || 'No hubo ACK válido'}. ${pnHttp}. ${pnDetail}`
            });
              ...
              if (uuidCell) {
                uuidCell.innerHTML = `
                  ...
                  <div class="small text-warning mt-1" style="white-space:normal;">
                    ...
                    PayNotify falló: ${escapeHtml(js.pay_notify_error || pnMsg || 'sin detalle')}
                    ${js.pay_notify_http_code ? ` (HTTP ${js.pay_notify_http_code})` : ''}
                  </div>
                `;
              }
            }
              ...
              if (uuidCell) {
                uuidCell.innerHTML = `
                  ...
                  <div class="small text-success mt-1" style="white-space:normal;">
                    ...
                    PayNotify OK: ${escapeHtml(pnMsg || 'confirmado')}
                    ${js.pay_notify_http_code ? ` (HTTP ${js.pay_notify_http_code})` : ''}
                  </div>
                `;
              }
  ...
  async function renderReports() { 
    ...
    invoiceCsvBtn.addEventListener('click', () => {
      ...
      const headers = ['ticket_no','plate','fecha','total','receptor','uuid','status'];
      const csv = [
        headers.join(','),
        ...rows.map(r => [
