...
async function renderReports() {
  ...
  // ✅ PDF emitidas SOLO binario G4S (sin BD)
  invoiceRowsEl.addEventListener('click', async (ev) => {
    const btn = ev.target.closest('button[data-action="pdf"]');
    if (!btn) return;
  ...
  // === Facturas manuales ===
  const fetchManualInvoiceReport = async () => {
    manualInvoiceRowsEl.innerHTML = `<tr><td colspan="8" class="text-center text-muted">Consultando...</td></tr>`;
    manualInvoiceSummaryEl.innerHTML = '';
    manualInvoiceMsgEl.textContent = '';
    clearAlert(manualInvoiceAlertEl);
    manualInvoiceCsvBtn.disabled = true;
    ...
    try {
      ...
      manualInvoiceRowsEl.innerHTML = `<tr><td colspan="8" class="text-center text-danger">Error: ${escapeHtml(err.message)}</td></tr>`;
      showAlert(manualInvoiceAlertEl, `No se pudo cargar el reporte: ${err.message}`, 'danger');
    }
  };
  ...
  const renderManualInvoiceRows = () => {
    const { slice } = paginate(
      manualInvoiceState.rows,
      manualInvoiceState.page,
      manualInvoiceState.perPage
    );

    if (!slice.length) {
      manualInvoiceRowsEl.innerHTML = `<tr><td colspan="8" class="text-center text-muted">No hay registros.</td></tr>`;
      manualInvoicePaginationEl.innerHTML = '';
      return;
    }
    ...
    manualInvoiceRowsEl.innerHTML = slice.map((r) => {
      ...
      // ✅ CAMBIO: tomar UUID igual que emitidas
      const uuidManual = getUuid(r);

      const pdfCell = uuidManual
        ? `
          <button type="button"
                  class="btn btn-sm btn-outline-primary"
                  data-action="pdf-manual"
                  data-uuid="${escapeHtml(uuidManual)}">
            PDF
          </button>`
        : `<span class="badge text-bg-light border">Sin UUID</span>`;
  ...
  // ✅ CAMBIO: PDF manual por UUID consultando a G4S (igual que tickets)
  manualInvoiceRowsEl.addEventListener('click', async (ev) => {
    const btn = ev.target.closest('button[data-action="pdf-manual"]');
    if (!btn) return;
    ...
    try {
      ...
      const blob = await resp.blob();
      downloadBlob(blob, `manual-${uuid}.pdf`);
      showAlert(manualInvoiceAlertEl, 'PDF generado desde G4S por UUID.', 'success');
    } catch (e) {
      showAlert(manualInvoiceAlertEl, `No se pudo generar el PDF: ${e.message}`, 'danger');
    } finally {
      btn.textContent = old;
  ...
  manualInvoiceCsvBtn.addEventListener('click', () => {
    ...
    const headers = ['id','fecha','monto','motivo','modo','nit','status'];
    const csv = [
      headers.join(','),
      ...rows.map(r => [
        String(r.id ?? '').replaceAll('"','""'),
